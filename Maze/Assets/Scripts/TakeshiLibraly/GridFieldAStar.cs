    ...
    public class GridFieldAStar
    {
        ...
        public bool AStarPath(GridFieldMap map, Coord position, Coord targetPos)
        {
            ...
            while (count < m_searchLimit)
            {
                ...
                //Debug.Log(minCell.position.x + "," + minCell.position.z 
                //    + " \n合計コスト： " + minCell.sumCost
                //    + " \n想定コスト： " + minCell.heuristicCost
                //    + " \n実コスト  ： " + minCell.cost);
        ...
        private CellInfo GetSatrtCell()
        {
            ...
            startCell.cost = 0;
            startCell.heuristicCost = Vector2Int.Distance(new Vector2Int(start.x,start.z), new Vector2Int(goal.x, goal.z));
        ...
        public void OpenCell(CellInfo center)
        {
            ...
            for (int x = -1; x <= 1; x++)
            {
                for (int z = -1; z <= 1; z++)
                {
                    ...
                    aroundCell.heuristicCost = Vector2Int.Distance(new Vector2Int(searchPos.x, searchPos.z), new Vector2Int(goal.x, goal.z));
                    ...
                    if (openList.Any(c => c.position == aroundCell.position))
                    {
                    ...
                    if (closeList.Any(c => c.position == aroundCell.position))
                    {
